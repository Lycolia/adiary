...
sub parse_inline {
	...
	foreach(@$lines) {
		...
		# 自動リンク記法
		# [^'"> ]を[^'"<> ]にすると一応動くが、Markdownリンクが壊れる
		# 因みにこの段階で<p>https://...</p>で入ってくるっぽい
		# [httpsを弾けば一旦は回避できそうで、pタグが破壊されるのでうまくいかない
		# 根本のパーサーから作り直したほうがいいかもしれない
...
sub escape_in_code {
	my $self = shift;
	foreach(@_) {
# TODO: なぜか<@>を書いた場合に不正にampが入りすぎるので消しておく、多分原因はここではないどこかにあると思うが特定できないため
# メールアドレス難読化エンコードは関係なかった
#		$_ =~ s/&/&amp;/g;
		$_ =~ s/</&lt;/g;
		$_ =~ s/>/&gt;/g;
	}
